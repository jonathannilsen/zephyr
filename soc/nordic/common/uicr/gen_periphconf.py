...
if TYPE_CHECKING:
    sys.path.insert(0, str(Path(__file__).parents[4] / "scripts/dts/python-devicetree"))
...
NON_PROG_OWNER_COMPAT_PATTERN = "^" + "|".join(NON_PROG_OWNER_COMPATS) + "$"
...
def periphconf_from_devicetree(
    edt_pickle_file: Path,
    product: Product,
    output_file: Path,
    output_debug_file: Path | None = None,
) -> None:
...
def address_is_global_periph(address: int) -> bool:
    ...
    return AddressRegion.from_address(address) in (AddressRegion.PERIPHERAL, AddressRegion.STM)


class UicrMatcher:
    def __init__(self, builder: uicr.Uicr, product: Product, processor: ProcessorID) -> None:
        self.uicr_address = 0
        ...
        try:
            ...
            raise RuntimeError(f"Processor ID {self._processor} does not correspond to an owner")
        try:
            self._domain = DomainID.from_processor(self._processor)
        except Exception:
            raise RuntimeError(f"Processor ID {self._processor} does not correspond to a domain")
    ...
    def visit_owned_partitions(self, node: edtlib.Node, /) -> None:
        ...
        end_addresses = [
            uicr.Value(a.val + p.regs[0].size, a.src) for a, p in zip(abs_addresses, partitions)
        ]
    ...
    def _parse_access_props(self, node: edtlib.Node) -> list[uicr.Memory.Access]:
        ...
        if "nordic,access" not in props:
            ...
            for owner, permission in batched(nordic_access, 2):
                ...
                access = uicr.Memory.Access(
                    owner=owner,
                    read=uicr.Value(NordicAccessPerm.NRF_PERM_R in access_perm, permission.src),
                    write=uicr.Value(NordicAccessPerm.NRF_PERM_W in access_perm, permission.src),
                    execute=uicr.Value(NordicAccessPerm.NRF_PERM_X in access_perm, permission.src),
                    secure=uicr.Value(NordicAccessPerm.NRF_PERM_S in access_perm, permission.src),
    ...
    def visit_nrf_dppic(self, node: edtlib.Node, /) -> None:
        ...
        for num, secure in channel_groups:
            self._builder.add_dppi_channel_group(addr, uicr.Feature(num=num, secure=secure))
        ...
        for num in source_channels:
            self._builder.add_dppi_link(
                addr,
                uicr.DppiLink(num, uicr.Value(uicr.DppiLink.Direction.SOURCE, src=num.src)),
            )

        for num in sink_channels:
            self._builder.add_dppi_link(
                addr,
                uicr.DppiLink(num, uicr.Value(uicr.DppiLink.Direction.SINK, src=num.src)),
            )
    ...
    def visit_ipc_icmsg(self, node: edtlib.Node, /) -> None:
        ...
        if tx_processor.val != ProcessorID.SECURE or self._processor == ProcessorID.SYSCTRL:
    ...
    def visit_peripheral_with_gpios(self, node: edtlib.Node, /) -> None:
        ...
        for name in node.props:
            ...
            for entry in dt_array_prop(node, name):
                ...
                ctrlsel = dt_lookup_ctrlsel(self._product, node.props[name], (port, num.val))
...
def dt_array_prop(
    ...
    return [uicr.Value(v, src=dt_to_unit_src((prop, i))) for i, v in enumerate(prop.val)]
    ...
    while n is not None:
        if "nordic,owned-partitions" in n.compats or "nordic,owned-memory" in n.compats:
            if "nordic,access" in n.props:
                ...
                secure = bool(n.props["nordic,access"].val[1] & NordicAccessPerm.NRF_PERM_S)
            else:
    ...
    *,
    ...
    for irq in node.interrupts:
        ...
        for processor_id in irq_processors:
            processors.append(uicr.Value(processor_id.value, src=dt_to_unit_src(irq_ctrl)))

    if not processors:
        try:
            node_processors = processor_ids_from_nodelabels(node.labels)
            for processor_id in node_processors:
                processors.append(uicr.Value(processor_id.value, src=dt_to_unit_src(node)))
        except ValueError:
            pass
...
def dt_processor_id(devicetree: edtlib.EDT) -> ProcessorID:
    ...
    try:
        ...
        raise RuntimeError(f"Devicetree 'cpu' node has invalid Processor ID {cpus[0].regs[0].addr}")
...
def dt_lookup_ctrlsel(
    ...
    if identifier.val in lut:
        ...
        if sub_entry in ident_lut:
            ...
            sub_entry_no_fun = NrfPsel(fun=FUN_IGNORE, port=sub_entry.port, pin=sub_entry.pin)
